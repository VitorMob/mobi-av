...
#define SwapEndian64(swap)           \
        htobe32(swap->sh_name);      \
        htobe32(swap->sh_type);      \
        htobe32(swap->sh_link);      \
        htobe32(swap->sh_info);      \
        htobe64(swap->sh_flags);     \
        htobe64(swap->sh_addr);      \
        htobe64(swap->sh_offset);    \
        htobe64(swap->sh_addralign); \
        htobe64(swap->sh_entsize);   \
        htobe64(swap->sh_offset);    \
        htobe64(swap->sh_size);
...
#define SwapEndian32(swap)           \
        ...
        htobe32(swap->sh_link);      \
        htobe32(swap->sh_info);      \
        htobe32(swap->sh_flags);     \
        htobe32(swap->sh_addr);      \
        htobe32(swap->sh_addralign); \
        htobe32(swap->sh_entsize);   \
        htobe32(swap->sh_offset);    \
        htobe32(swap->sh_size);
...
void ElfSection::MapElfSection(const struct FileElf &p_fileElf)
{
        ...
        else if (p_fileElf.buffer[EI_CLASS] == ELFCLASS64)
        {
                ...
                for (int i = 0; i < p_fileElf.elf.Header64->e_phnum; i++)
                {
                        ...
                        std::string name;
                        int lenght = (p_fileElf.elf.Header64->e_shstrndx + p_fileElf.elf.Section64.back()->sh_name);

                        if (p_fileElf.buffer[EI_DATA] != ELFDATANATIVE)
                        {
                                SwapEndian64(p_fileElf.elf.Section64.back());
                        }
                }
        ...
        else
        {
                ...
                for (int i = 0; i < p_fileElf.elf.Header32->e_phnum; i++)
                {
                        ...
                        if (p_fileElf.buffer[EI_DATA] != ELFDATANATIVE)
                        {
                                SwapEndian32(p_fileElf.elf.Section32.back());
                        }
                }
