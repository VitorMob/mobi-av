...
void *Arena::req(unsigned int p_amount)
{
  if (p_amount <= m_amount)
  {
    ...
    {
      realloc(m_limit,  m_limit);
      goto new_block;
    }
    else
      goto new_block;
  }
  else if (p_amount > m_size && m_overwrite )
  {
    realloc(p_amount, m_limit);
    goto new_block;
  }
  else
  {
 ...
 * ... [  ][  ][  ] <- [delete] - [alocate more] -> [  ][  ][  ][  ] +++
 * */
void Arena::realloc(unsigned int p_amount, unsigned int p_limit)
{
  if (p_amount > 0 && p_amount > m_size &&  p_limit > 0)
  {
    if(m_mem != nullptr)
      delete[] m_mem;
